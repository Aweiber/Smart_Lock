...
#define PIN_INT0_IRQn   EXTI9_5_IRQn
       ...
       if((Lpcd.Calibration.Value > Lpcd.Calibration.Range_L)&&(Lpcd.Calibration.Value < Lpcd.Calibration.Range_H)) 
         {
					if((Lpcd.Calibration.Value - (Lpcd.Fullscale_Value>>2) > 0 )&&((Lpcd.Calibration.Value + (Lpcd.Fullscale_Value>>2) ) < Lpcd.Fullscale_Value)) //   
           {
						 ...
						 #if 0
               Lpcd.Calibration.Threshold_Max = Lpcd.Calibration.Value - 10;//+ 15
               Lpcd.Calibration.Threshold_Min = Lpcd.Calibration.Value - 15;//- 15
						 #else 
               Lpcd.Calibration.Threshold_Max = Lpcd.Calibration.Value - 25;//- 30;//+ 15
               Lpcd.Calibration.Threshold_Min = Lpcd.Calibration.Value - 60;//- 15
...
{ 
        ...
        FM175XX_HPD(0);//进入LPCD模式	  
				HAL_NVIC_EnableIRQ(EXTI0_1_IRQn);//MCU打开中断
        }
    else
        {               
				FM175XX_HPD(1);//退出LPCD模式	
        Lpcd_Set_IE(0);//FM75XX关闭LPCD中断
				HAL_NVIC_DisableIRQ(EXTI0_1_IRQn);//MCU关闭中断
        }     
    return;
}
...
{
	unsigned char result;
	result=Add_TyteB_Card();
	if(result==OK)
	{
		printf("IC卡是B卡类型\n");
		return result;
	}
	result=Add_TyteA_Card();
	if (result == OK)
	{
	  printf("IC卡是A卡类型\n");
	}
