...
void wifi_uart_write_frame(unsigned char fr_type, unsigned short len)
{
    ...
    wifi_uart_write_data((unsigned char *)wifi_uart_tx_buf, len);
	  ...
	  for (int i = 0; i < len; i++)
	  {
			printf("0x%x ", wifi_uart_tx_buf[i]);
	  }
		printf("\r\n");
}
...
//    unsigned char dp_num;
    ...
    switch(cmd_type)
    {
        ...
        case WIFI_STATE_CMD:                                   //wifi工作状态	
            ...
            wifi_uart_write_frame(WIFI_STATE_CMD,0);
				    switch (wifi_work_state)
						{
							case 0:
								printf("smartconfig配置状态\n");
							  if (Lusers.APMData.Show_Page == OLEDAPPPAIR)
								{
									LED1_OFF;
									delay_ms(50);
									LED1_ON;
					      ...
					      break;
							case 2:
							  ...
							  if (Lusers.APMData.Show_Page == OLEDAPPPAIR)
								...
								Lusers.APMData.wifi_risi = 1;
								break;
							case 3:
								printf("模组已连接上路由器，并已连上网\n");
								break;
							case 4:
								printf("模组已连上云端\n");
							  if (Lusers.APMData.Show_Page == OLEDAPPPAIR)
								{									
									...
									Lusers.Application.ONOFFLed(LED_OFF);
									Lusers.Application.OLED_Show(OLEDAPPPAIRSUCCESS);
									...
									mcu_dp_raw_update(DPID_UPDATE_ALL_PASSWORD, pwdzonepage, pwd_lenth[0]);           //上报密码编号
									...
									mcu_dp_raw_update(DPID_UPDATE_ALL_CARD, nfczonepage, nfc_lenth[0]);           //上报卡编号
									...
									mcu_dp_raw_update(DPID_UPDATE_ALL_FIN_VEIN, fingerzonepage, finger_lenth[0]);		  //上报指静脉编号						 	
									
									delay_ms(1000);
									LED1_ON;
									Lusers.APMData.Show_Page = OLEDAPPMENU1;
								}
                Send_Get_Wifi_Risi();     //查询当前wifi信号							  
								break;
							default:
								break;
						}
        ...
        case MUL_TEMP_PASS_CMD:                                 //请求云端临时密码（支持多组）
					  if (Lusers.APMData.Show_Page == OLEDTEMPORARYWAIT)
						{
        ...
        case ROUTE_RSSI_CMD:
					Get_Wifi_Risi(wifi_data_process_buf[offset + 6], wifi_data_process_buf[offset + 7]);
					break;
