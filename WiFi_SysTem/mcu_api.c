 ...
 * @note   Null
 */
unsigned char mcu_dp_value_update(unsigned char dpid,unsigned long value,unsigned char time_state)
{
    unsigned short length = 0;
    ...
    if(stop_update_flag == ENABLE)
		...
		if (time_state == NEED_TIME_UPDATA)
		{
		...
		else if (time_state == NOT_TIME_UPDATA)
		{
			time[0] = 0;
		}
		length = set_wifi_uart_buffer(length, time, 7);
    ...
    while((rx_in - offset) >= PROTOCOL_HEAD) {
        ...
        if(get_check_sum((unsigned char *)wifi_data_process_buf + offset,rx_value_len - 1) != wifi_data_process_buf[offset + rx_value_len - 1]) {
            ...
            continue;
				...
				printf("处理一帧数据 ：");
				for(int i = 0; i < rx_value_len; i++)
				{
					printf("0x%x ", wifi_data_process_buf[i]);
				}
				printf("\r\n");
        data_handle(offset);
        offset += rx_value_len;
