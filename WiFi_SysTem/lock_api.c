    ...
    switch (status) {
        case 0: {
            printf("动态密码验证通过\n");
					  ...
					  Lusers.Application.OLED_Show(OLEDDYMANICPASS);
					  Lusers.Application.Motor_Play(MOTOR_OPEN);
						...
						mcu_dp_value_update(DPID_UNLOCK_DYNAMIC, 0, NEED_TIME_UPDATA);				  
            break;
        }
            ...
            printf("动态密码验证失败\n");
					  Lusers.APMData.test_error ++;
						...
						else
						{
							...
							Lusers.Application.ONOFFLed(LED_OFF);
							Lusers.Application.OLED_Show(OLEDDYMANICFAIL);
							delay_ms(1000);
 ...
 */
uint8_t mul_temp_pass_data(unsigned char suc_flag, unsigned char pass_ser, 
...
//    #error "请自行完成多组临时密码信息处理代码,并删除该行"
   ...
   */
    if (suc_flag == 1) {
			printf("获取多组临时密码成功\n");
			uint32_t start_time = 0, now_time = 0, end_time = 0;
			uint8_t temp_time[6] = {0};
			Lusers.Application.GetTime();
			temp_time[0] = hymd.Year;
			temp_time[1] = hymd.Month;
			temp_time[2] = hymd.Date;
			temp_time[3] = hhms.Hours;
			temp_time[4] = hhms.Minutes;
			temp_time[5] = hhms.Seconds;
			BeiJTime_To_GelinUTC(temp_time);
      
			start_time = gl_start[0]*12*30*24*60 + gl_start[1]*30*24*60 + gl_start[2]*24*60 + gl_start[3]*60 + gl_start[4];
			now_time = temp_time[0]*12*30*24*60 + temp_time[1]*30*24*60 + temp_time[2]*24*60 + temp_time[3]*60 + temp_time[4];
			end_time = gl_end[0]*12*30*24*60 + gl_end[1]*30*24*60 + gl_end[2]*24*60 + gl_end[3]*60 + gl_end[4];
			
			printf("start_time = %d, now_time = %d, endtime = %d", start_time, now_time, end_time);
			if ((now_time >= start_time) && (now_time < end_time))
			{
				printf("临时密码时间正确\n");
				for (uint8_t i = 0; i < 7; i++)
				{
					if (Lusers.APMData.Temparoary_Pwd_Bufer[i] == pass[i])
					{
						if (i == 6)
						{
							return SUCCESS;
...
void mul_temp_pass_handle(const unsigned char data[])
{
    ...
    if (suc_flag == 1) {
			  if ((pass_num == 0) || (pwd_state == 1))
				...
				{
					uint8_t Temp_Pass = FALSE;
					for (i=0;i<pass_num;i++) {
							offset = i*(15+pass_len);
							Temp_Pass = mul_temp_pass_data(suc_flag, data[offset+3], data[offset+4], data[offset+5], 
																 data+offset+6, data+offset+12, data+offset+18, pass_len);
						  if (Temp_Pass == SUCCESS)
							{
								break;
							}
					}
					if (Temp_Pass == SUCCESS)
					{
					  ...
					  Lusers.Application.Motor_Play(MOTOR_OPEN);
						...
						Lusers.Application.OLED_Show(OLEDCLEAR);
						Lusers.APMData.Show_Page = OLEDCLEAR;
						mcu_dp_value_update(DPID_UNLOCK_TEMPORARY, data[offset+3]+900, NEED_TIME_UPDATA);					
					}
					else
					{
						printf("临时密码校验失败\n");
